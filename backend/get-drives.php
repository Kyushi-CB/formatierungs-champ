...
# if POST from GUI = "format" -> start formatting
if($_POST['action'] == 'format') {

  # TODO: NVME support currently missing, only sata on /dev/sd*#
  # TODO: query if drives are locked or frozen and remove that state automatically
  # TODO: check if enhanced security erase is supported
  # get array of paths for current available drives on /dev/sd* 
  ...
  for ($i = 0; $i < count($drivePath); $i++) {
    ...
    #check if SSD, non SSD or N/A
    if ($getType != "" && $getType == "Nominal Media Rotation Rate: Solid State Device" || $getType == "Nominal Media Rotation Rate: Solid State Device") {
